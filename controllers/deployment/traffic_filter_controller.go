...
// TrafficFilterReconciler reconciles a TrafficFilter object
type TrafficFilterReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=deployment.ec.kubeform.com,resources=trafficfilters,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=deployment.ec.kubeform.com,resources=trafficfilters/status,verbs=get;update;patch

func (r *TrafficFilterReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("trafficfilter", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch TrafficFilter")
...
func (r *TrafficFilterReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &deploymentv1alpha1.TrafficFilter{}); err != nil {
			klog.Error(err, "unable to set up auditor", deploymentv1alpha1.TrafficFilter{}.APIVersion, deploymentv1alpha1.TrafficFilter{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&deploymentv1alpha1.TrafficFilter{}).
