...
// ElasticsearchKeystoreReconciler reconciles a ElasticsearchKeystore object
type ElasticsearchKeystoreReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=deployment.ec.kubeform.com,resources=elasticsearchkeystores,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=deployment.ec.kubeform.com,resources=elasticsearchkeystores/status,verbs=get;update;patch

func (r *ElasticsearchKeystoreReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("elasticsearchkeystore", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ElasticsearchKeystore")
...
func (r *ElasticsearchKeystoreReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &deploymentv1alpha1.ElasticsearchKeystore{}); err != nil {
			klog.Error(err, "unable to set up auditor", deploymentv1alpha1.ElasticsearchKeystore{}.APIVersion, deploymentv1alpha1.ElasticsearchKeystore{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&deploymentv1alpha1.ElasticsearchKeystore{}).
...
func (r *ElasticsearchKeystoreReconciler) SensitiveSecretWatch(ctx context.Context) handler.MapFunc {
	...
	return func(o client.Object) []ctrl.Request {
		...
		if !ok {
			log.Error(fmt.Errorf("expected a Secret but go a %T", o), "failed to get secret for ElasticsearchKeystore")
			return nil
		}
		...
		resourceList := &deploymentv1alpha1.ElasticsearchKeystoreList{}
		if err := r.List(ctx, resourceList, client.InNamespace(secNamespace)); err != nil {
			log.Error(err, "failed to list ElasticsearchKeystore")
			return nil
		}
